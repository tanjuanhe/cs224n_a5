...
if args.function == 'pretrain':
    ...
    text2 = open(args.pretrain_corpus_path, encoding='utf-8').read()
    train_dataset = dataset.CharCorruptionDataset(text2, block_size)
    ...
    text2 = open(args.finetune_corpus_path, encoding='utf-8').read()
    train_dataset = dataset.NameDataset(pretrain_dataset, text2)
    
    tconf = trainer.TrainerConfig(max_epochs=epochs, batch_size=256, learning_rate=args.finetune_lr,
                      lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,
                      num_workers=4, writer=writer)
    trainer = trainer.Trainer(model, train_dataset, None, tconf)
    trainer.train()
